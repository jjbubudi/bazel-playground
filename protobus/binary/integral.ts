...
export function decodeInt32(offset: number, bytes: Readonly<Uint8Array>): Decoded<Int32> {
...
export function decodeBoolean(offset: number, bytes: Readonly<Uint8Array>): Decoded<boolean> {
  const result = decodeUint32(offset, bytes)[0] === 1 ? true : false;
  return [result, 1];
}
